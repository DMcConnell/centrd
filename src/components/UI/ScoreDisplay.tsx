...
export const ScoreDisplay: React.FC<ScoreDisplayProps> = ({
  ...
  const { getBestScoreForDifficulty, getHighScoresForDifficulty } =
    useScoring();
        ...
        <div className='high-scores'>
          ...
          <ol>
            {highScores.slice(0, 5).map((score, index) => (
              <li
                key={index}
                className={score === averageScore ? 'current-score' : ''}
              >
                {score.toFixed(2)}
              </li>
            ))}
